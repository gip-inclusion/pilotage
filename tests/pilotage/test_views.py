...
def test_mentions_legales(snapshot, client):
    assert str(parse_response_to_soup(client.get(reverse("pilotage:mentions_legales")), selector="#main")) == snapshot()


def test_politique_de_confidentialite(snapshot, client):
    assert str(parse_response_to_soup(client.get(reverse("pilotage:politique_de_confidentialite")), selector="#main")) == snapshot()

def test_inscription_lettre_information(snapshot, client):
    assert str(parse_response_to_soup(client.get(reverse("pilotage:inscription_lettre_information")), selector="#main")) == snapshot()

def test_tableaux_de_bord_prives(snapshot, client):
    assert str(parse_response_to_soup(client.get(reverse("pilotage:tableaux_de_bord_prives")), selector="#main")) == snapshot()
